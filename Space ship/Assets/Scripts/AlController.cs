...
namespace SpaceShooter
{
    ...
    public class AlController : MonoBehaviour
    {
        ...
        [SerializeField] private AIPointPatrol[] m_Route;
        ...
        private void ActionFindNewMovePosition()
        {
            ...
            if(m_AIBehaviour == AIBehaviour.Route)
            {
                bool isInsidePatrolZone = (m_Route[RouteCounter].transform.position - transform.position).sqrMagnitude < m_Route[RouteCounter].Radius * m_Route[RouteCounter].Radius;

                if ((isInsidePatrolZone == true) && (RouteCounter != m_Route.Length - 1))
                {
                    RouteCounter++;
                }

                if ((isInsidePoint(m_Route[m_Route.Length - 1]) == true))
                {
                    RouteCounter = 0;
                }

                m_MovePosition = m_Route[RouteCounter].transform.position;
            }
        }
        ...
        private bool isInsidePoint(AIPointPatrol point)
        {
            return (point.transform.position - transform.position).sqrMagnitude < point.Radius * point.Radius;
        }

        private Vector3 MakeLead(Destructible selectedTarget)
        {
            Rigidbody2D rd = selectedTarget.GetComponent<Rigidbody2D>();
            Vector3 lead = selectedTarget.transform.up * rd.velocity;
            return selectedTarget.transform.position + lead;
        }
